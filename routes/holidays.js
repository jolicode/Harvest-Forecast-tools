...
router.get(['/', '/:start/:end'], (req, res, next) => {
  ...
  if (!_.has(req, 'session.harvest.token') || !_.has(req, 'session.forecast.token')) {
    return res.redirect('/');
  }

  if (req.params.start !== undefined) {
    start = moment.utc(req.params.start + ' +0000', 'YYYY-MM-DD Z').startOf('day');
  } else {
  ...
  if (req.params.end !== undefined) {
    ...
    end = moment().utc().endOf('month').endOf('day');
  }

  let prevStart = start.clone().subtract(1, 'month').startOf('month').startOf('day');
  let prevEnd = prevStart.clone().endOf('month').endOf('day');
  let nextEnd = end.clone().add(1, 'month').endOf('month').endOf('day');
  let nextStart = nextEnd.clone().startOf('month').startOf('day');

  let harvestFetcher = new HarvestFetcher(req.storage, process.env.harvest_client_domain, req.session.harvest.token);
  ...
  Promise.all([
    ...
    res.render('holidays', {
      title: 'Holidays summary (' + start.format('YYYY-MM-DD') + ' → ' + end.format('YYYY-MM-DD') + ')',
      holidays: extractHolidays(
        start,
        end,
        values[0],
        values[1],
        process.env.harvest_holiday_projects.split(",").map(id => parseInt(id))
      ),
