...
const getValidators = () => {
  return [
    ...
    check('project')
      .not()
      .isEmpty()
      .withMessage('please choose one project'),
    check('people')
      .not()
      .isEmpty()
      .withMessage('should contain at least one user'),
    check('duration')
      .isInt({ min: 1, max: 8 })
      .withMessage('should be an integer (between 1 and 8 hours).'),
    check('people.*')
      .isBoolean()
      ...
      .custom((value, { req, location, path }) => {
        return new Promise((resolve, reject) => {
          ...
          forecastFetcher.getPeople().then(data => {
            ...
            let checkedUserIds = _.map(_.keys(value), v => {
              return parseInt(v, 10);
            });
            if (
              !_.isEmpty(checkedUserIds) &&
              _.isEmpty(_.difference(checkedUserIds, userIds))
            ) {
              resolve(true);
            } else {
              reject(true);
    ...
    check('preexisting_assignement')
      .custom((values, { req, location, path }) => {
        return new Promise((resolve, reject) => {
          let values = req.body;
          if (_.get(values.in, 'forecast', false)) {
            ...
            forecastFetcher.getAssignements(date, date).then(assignments => {
              let checkedUserIds = _.map(_.keys(values.people), v => {
                return parseInt(v, 10);
              });
              _.each(checkedUserIds, userId => {
                if (
                  _.find(assignments, {
                    person_id: userId,
                    project_id: parseInt(values.project, 10),
                  })
                ) {
                  return reject(true);
                }
      ...
      .withMessage(
        'At least one of the selected users already has a Forecast assignement for this project on that day.'
      ),
  ];
};
...
router.post('/', getValidators(), (req, res, next) => {
  try {
    ...
    Promise.all([
      forecastFetcher.getPeopleByIds(
        _.map(_.keys(values.people), v => {
          return parseInt(v, 10);
        })
      ),
      ...
      .then(data => {
        ...
        if (_.get(values.in, 'forecast', false)) {
          _.each(data[0], user => {
            addPromises.push(
              forecastFetcher.addAssignement({
