...
router.get(['/', '/:start/:end'], (req, res, next) => {
  ...
  let prettyDays = [];
  let days = moment.recur(start, end).every(1).day().all();
  _.each(days, day => {
    prettyDays.push({
  ...
  Promise.all([
    ...
    res.render('diff', {
      title: 'Harvest / Forecast diff (' + start.format('YYYY-MM-DD') + ' → ' + end.format('YYYY-MM-DD') + ')',
      prettyDays,
      users: reconcilier(
        start,
        end,
        values[0],
        values[1],
        process.env.harvest_nonreconciliable_client_ids.split(",").map(id => parseInt(id))
      ),
