...
const extractHolidays = (start, end, harvestData, forecastData, holidayProjectIds) => {
  // harvest analysis
  ...
  let harvestappHolidays = holidaysAsStringSelector(
    harvestHolidaysSelector(harvestData[2], harvestappProjects, holidayProjectIds)
  );
  ...
  let forecastAssignements = forecastAssignementsSelector(forecastData[2], start, end, forecastWorkingDays, forecastProjects);
  let forecastUsers = forecastUsersSelector(forecastData[0]);
  let forecastHolidays = holidaysAsStringSelector(
    forecastHolidaysSelector(forecastAssignements, forecastUsers, harvestappProjects, holidayProjectIds)
  );
  ...
  return _.filter(harvestappUsers, u => { return _.has(u, 'harvestHolidays') || _.has(u, 'forecastHolidays') });
};

const reconcilier = (start, end, harvestData, forecastData, nonreconciliableClientIds) => {
  let days = moment.recur(start, end).every(1).day().all('YYYY-MM-DD');
  ...
  let assignements = forecastAssignementsSelector(forecastData[2], start, end, workingDays, projects);
  let data = forecastUsersSelector(forecastData[0]);

  // harvest analysis
  let harvestappProjects = harvestProjectsSelector(harvestData[1]);
  let harvestappAssignements = harvestAssignementsSelector(harvestData[2], harvestappProjects);
  ...
  _.each(data, user => {
    ...
    if (_.has(harvestappAssignements, String(user.harvest_user_id))) {
      user.harvestAssignements = harvestappAssignements[String(user.harvest_user_id)];
    }
  });
  ...
  data = _.filter(
    data,
    (user) => { return (user.forecastAssignements || user.harvestAssignements)}
  );
  ...
  _.each(data, user => {
    ...
    _.each(days, day => {
      ...
      let forecastAssignement = _.get(user, ['forecastAssignements', day], null);
      let harvestappAssignement = _.clone(_.get(user, ['harvestAssignements', day], null));

      if (forecastAssignement === null && harvestappAssignement === null) {
        error = 0;
      } else if (forecastAssignement === null || harvestappAssignement === null) {
        error = 2;
      } else {
        error = 0;
        _.each(forecastAssignement, forecastTask => {
          let clientId = _.find(clients, o => { return o.id === forecastTask.project.client_id }).harvest_id;

          // search for equivalent harvest assignement (based on harvest_id + duration)
          let equivalentHarvestProject = _.find(harvestappAssignement, { project_id: forecastTask.project.harvest_id, allocation: forecastTask.allocation });
          let equivalentHarvestProjectByClient;

          if (!_.includes(nonreconciliableClientIds, clientId)) {
            equivalentHarvestProjectByClient = _.find(harvestappAssignement, { client_id: clientId, allocation: forecastTask.allocation });
          }
