...
const extractHolidays = (
  ...
  let harvestappHolidays = holidaysAsStringSelector(
    harvestHolidaysSelector(harvestData[2], holidayProjectIds)
  );
...
const reconcilier = (
  ...
  let users = forecastUsersSelector(forecastData[0]);

  // harvest analysis
  let harvestappAssignements = harvestAssignementsSelector(harvestData[2]);
  ...
  _.each(users, user => {
    ...
    _.each(days, day => {
      ...
      if (forecastAssignement === null && harvestappAssignement === null) {
        ...
        // clear up 0-hours assignments
        harvestappAssignement = _.reject(harvestappAssignement, [
          'allocation',
          0,
        ]);
