...
const activeUsersSelector = (users) => {
  return _.sortBy(_.compact(_.reject(users, 'archived')), ['first_name', 'last_name']);
}

const assignementsSelector = (assignements, start, end, workingDays, projects) => {
  let data = {};
  ...
  _.each(assignements, assignement => {
    ...
    if (formatedStart > assignement.end_date || formatedEnd < assignement.start_date) {
      return;
    }
    ...
    let days = moment.recur(assignement.start_date, assignement.end_date).every(workingDays[assignement.person_id]).daysOfWeek().all('YYYY-MM-DD');
...
const holidaysSelector = (assignements, users, harvestProjects, holidayProjectIds) => {
  let data = {};
    ...
    _.each(userAssignements, userDayAssignements => {
      _.each(userDayAssignements, assignement => {
        if (_.includes(holidayProjectIds, assignement.project.harvest_id)) {
          let projectName = harvestProjects[assignement.project.harvest_id].name;
...
const usersSelector = (users) => {
  return _.sortBy(_.map(users, user => {
    return _.pick(user, ['id', 'first_name', 'last_name', 'avatar_url', 'harvest_user_id', 'working_days']);
